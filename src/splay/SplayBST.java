...
/**
 * This program will authenticate the splay tree which
 * is also a binary tree. But this time it is covered 
 * with splay operations. There are two main operation
 * to control 1. zig zig and 2. zig zag
 * For zigzig there should be two level to the node and 
 * the path is a straight line. For zigzag it is usable
 * when the path from the node is not a direct line. 
 * as example when the node is left child of the node 
 * which is the right child of the root.
 * 
